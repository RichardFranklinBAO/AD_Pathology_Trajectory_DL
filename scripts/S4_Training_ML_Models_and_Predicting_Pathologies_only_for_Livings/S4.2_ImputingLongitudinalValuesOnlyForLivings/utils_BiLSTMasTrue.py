...
class LSTMModel(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.bidirectional:
            forward_hidden = hidden[-2]  # 最后一层正向隐藏状态
            backward_hidden = hidden[-1]  # 最后一层反向隐藏状态
...
def train_and_evaluate_model(
    ...
    weight_decay=1e-5, dropout_rate=0.5, bidirectional=True  # 新增bidirectional参数
    ):
    """
    ...
    Args:
        ...
        bidirectional: (bool): If True, the LSTM is bidirectional.
    ...
    model = LSTMModel(input_dim, hidden_size, output_dim, num_layers, dropout_rate, bidirectional=bidirectional)
...
def cross_validate_lstm(data, n_splits, input_dim, output_dim, num_epochs, patience, lr_scheduler_patience, lr_factor,
                        hidden_size, num_layers, learning_rate, batch_size, seed, dropout_rate, bidirectional = True):
    """
