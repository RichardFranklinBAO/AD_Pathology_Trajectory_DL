...
# BiLSTM & ReLU Model for sequence prediction

class LSTMModel(nn.Module):
    """
    An LSTM-based model for sequence prediction tasks with Batch Normalization.
    Now supports bidirectional LSTM and includes an extra ReLU activation layer.
    """
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers, dropout=0.5, bidirectional=True):
        ...
        self.relu = nn.ReLU()  # 额外的 ReLU 激活层
        self.fc = nn.Linear(hidden_dim * self.num_directions, output_dim)

    def forward(self, x):
        _, (hidden, _) = self.lstm(x)
        if self.bidirectional:
            forward_hidden = hidden[-2]  # 正向隐藏状态
            backward_hidden = hidden[-1]  # 反向隐藏状态
...
def train_and_evaluate_model(
        ...
        weight_decay=1e-5, dropout_rate=0.5, bidirectional = True):
    """
    ...
    model = LSTMModel(input_dim, hidden_size, output_dim, num_layers, dropout_rate, bidirectional = bidirectional).to(device)
    ...
    with torch.no_grad():
        for batch_x, batch_y in test_loader:
            outputs = model(batch_x)
            batch_y_cpu = batch_y.cpu() # Move tensor to CPU
            batch_y_df = pd.DataFrame(batch_y_cpu.numpy()) # Now convert to numpy
            # batch_y_df = pd.DataFrame(batch_y.numpy())

            outputs_cpu = outputs.cpu()  # Move tensor to CPU
            pred_y_df = pd.DataFrame(outputs_cpu.numpy()) # Now convert to numpy
...
def cross_validate_lstm(data, n_splits, input_dim, output_dim, num_epochs, patience, lr_scheduler_patience, lr_factor,
                        hidden_size, num_layers, learning_rate, batch_size, seed, dropout_rate, bidirectional):
    """
...
def select_lstm_hyperparameters(train_sequences, feature_columns, target_columns, 
                              ...
                              patience=10, lr_scheduler_patience=5, lr_factor=0.5, bidirectional = True):
    """
