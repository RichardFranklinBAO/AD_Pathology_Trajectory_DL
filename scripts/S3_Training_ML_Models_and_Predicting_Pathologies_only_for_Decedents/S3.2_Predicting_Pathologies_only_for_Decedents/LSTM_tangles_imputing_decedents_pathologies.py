...
# Output directory for Tangles predictions
OUTPUT_DIR = utils_training.RESULTS_DIR / "S3" / "S3.2" / "LSTM_tangles"
...
def run_imputation():
    ...
    # Pass 'bidirectional=False' because Tangles uses standard LSTM
    ...
    # 4. Save Results
    prefix = f"lstm_tangles_{'_'.join(TARGETS)}"
    results_df.to_csv(OUTPUT_DIR / f"{prefix}.csv", index=False)
    
    target_str = '_'.join(TARGETS)
    oof_df.sort_values(["projid", "fu_year"]).to_csv(
        OUTPUT_DIR / f"LSTM_tangles_clean_imputed_{target_str}_cv5_allsteps.csv", index=False
    )
