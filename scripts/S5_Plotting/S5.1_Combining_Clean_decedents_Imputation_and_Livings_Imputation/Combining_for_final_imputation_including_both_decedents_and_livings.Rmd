...
```{r}
# ---- 规范 + 校验列名 ----
names(combined) <- trimws(tolower(names(combined)))  # 去空格并小写
...
combined <- combined %>%
  ...
  dplyr::mutate(dplyr::across(all_of(required_pred), as.numeric)) %>%  # 预测值转数值
  dplyr::select(all_of(required_all), dplyr::everything()) %>%          # 固定列在前
